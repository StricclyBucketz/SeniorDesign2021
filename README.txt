...
7. In order to capture runtime data during testing rosbag may implemented in the following manner:
  ...
  rosbag record -a
  Ctrl-C when you would like the recording to end, the -a flag records all available topics, if you would like to record specific   topics you will need to use the 
  "rosbag record <topic-names>" convention
...
9. To convert the .txt file to a .csv file for easier import into numerical analysis software, you can use the script provided in our base directory with: 
   "./txt2csv.bash fileName.txt fileName.csv"
